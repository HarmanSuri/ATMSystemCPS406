...
public class MainFrame extends JFrame {
    ...
    static HashMap<Integer, Integer> bankDB = new HashMap<Integer, Integer>() {{put(8888, 1);}};
    static HashMap<Integer, Customer> custDB = new HashMap<Integer, Customer>() {{put(1, cust1);}};
    static Bank ATMBank = new Bank("Money Bank", "28 Wall St.", 0, bankDB, custDB);
    ...
    public void initialize() {
        ...
        for (int i = 0; i < 12; i++) {
            if (i < 9){keyButtons[i] = new JButton(String.valueOf(i+1));}
            else if (i == 9) {keyButtons[i] = new JButton(String.valueOf("."));}
            else if (i == 10) {keyButtons[i] = new JButton(String.valueOf("0"));}
            else {keyButtons[i] = new JButton(String.valueOf("00"));}
        }
        for (int i = 0; i < 12; i++) {
        ...
        options.add(cancel);options.add(clear);options.add(new JButton(""));options.add(ok);
    ...
    static ActionListener keylisten = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            if ((logDone == false) && (ptextfield.getText().length() < 4)){
                ...
                if(amount.length() < 6){
                    if((((e.getSource() == keyButtons[9]) && (countChar(amount, '.') < 1)) || ((e.getSource() != keyButtons[9])) && ((amount.indexOf(".") + 3 > amount.length()) || (amount.indexOf(".") == -1)))){
